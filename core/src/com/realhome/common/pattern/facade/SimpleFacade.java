...
package com.realhome.common.pattern.facade;

import com.realhome.common.pattern.command.Command;
import com.realhome.common.pattern.mediator.Mediator;
import com.realhome.common.pattern.mvc.CoreController;
import com.realhome.common.pattern.mvc.CoreModel;
import com.realhome.common.pattern.mvc.CoreView;
...
public class SimpleFacade implements Facade {
	 ...
	 * <p>
	 * <p>
	 * This <code>IFacade</code> implementation is a Singleton, so you should not call the constructor directly, but instead call
	 * the static Singleton Factory method <code>Facade.getInstance()</code> */
	 ...
	 * <p>
	 * <p>
	 * Called automatically by the constructor. Override in your subclass to do any subclass specific initializations. Be sure to
	 * call <code>super.initializeFacade()</code>, though.
	 * </P> */
	 ...
	 * <p>
	 * <p>
	 * Called by the <code>initializeFacade</code> method. Override this method in your subclass of <code>Facade</code> if one or
	 ...
	 * </UL>
	 * If you don't want to initialize a different <code>IController</code>, call <code>super.initializeController()</code> at the
	 * beginning of your method, then register <code>Command</code>s.
	 * </P> */
	 ...
	 * <LI>You have <code>Proxy</code>s to register with the Model that do not retrieve a reference to the Facade at construction
	 * time.</code></LI>
	 * </UL>
	 * If you don't want to initialize a different <code>IModel</code>, call <code>super.initializeModel()</code> at the beginning
	 * of your method, then register <code>Proxy</code>s.
	 * <p>
	 * Note: This method is <i>rarely</i> overridden; in practice you are more likely to use a <code>Command</code> to create and
	 * register <code>Proxy</code>s with the <code>Model</code>, since <code>Proxy</code>s with mutable tools will likely need to
	 * send <code>INotification</code>s and thus will likely want to fetch a reference to the <code>Facade</code> during their
	 * construction.
	 * </P> */
	 ...
	 * </UL>
	 * If you don't want to initialize a different <code>IView</code>, call <code>super.initializeView()</code> at the beginning of
	 * your method, then register <code>IMediator</code> instances.
	 * <p>
	 * Note: This method is <i>rarely</i> overridden; in practice you are more likely to use a <code>Command</code> to create and
	 * register <code>Mediator</code>s with the <code>View</code>, since <code>IMediator</code> instances will need to send
	 * <code>INotification</code>s and thus will likely want to fetch a reference to the <code>Facade</code> during their
	 * construction.
	 * </P> */
	 ...
	 * 
	 * @param proxy the name of the <code>IProxy</code> instance to be registered with the <code>Model</code>. */
