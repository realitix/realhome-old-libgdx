...
import com.realhome.common.pattern.observer.BaseObserver;
import com.realhome.common.pattern.observer.Notification;
import com.realhome.common.pattern.observer.Observer;
...
public class CoreView implements View {
	 ...
	 * <p>
	 * <p>
	 * This <code>IView</code> implementation is a Singleton, so you should not call the constructor directly, but instead call the
	 * static Singleton Factory method <code>View.getInstance()</code>
	 * 
	...
	public synchronized static CoreView getInstance () {
		if (instance == null) instance = new CoreView();
	 ...
	 * <p>
	 * <p>
	 * Called automatically by the constructor, this is your opportunity to initialize the Singleton instance in your subclass
	 ...
	 * <p>
	 * <p>
	 * All previously attached <code>Observers</code> for this <code>Notification</code>'s list are notified and are passed a
	 * reference to the <code>Notification</code> in the order in which they were registered.
	 * </P>
	 * 
	 * @param note the <code>Notification</code> to notify <code>Observers</code> of. */
	 ...
	 * <p>
	 * <P>
	 * Registers the <code>Mediator</code> so that it can be retrieved by name, and further interrogates the <code>Mediator</code>
	 * for its <code>Notification</code> interests.
	 * </P>
	 * <P>
	 * If the <code>Mediator</code> returns any <code>Notification</code> names to be notified about, an <code>Observer</code> is
	 * created encapsulating the <code>Mediator</code> instance's <code>handleNotification</code> method and registering it as an
	 * <code>Observer</code> for all <code>Notifications</code> the <code>Mediator</code> is interested in.
	 * </p>
	 * 
	 * @param mediator the name to associate with this <code>IMediator</code> instance */
	public void registerMediator (final Mediator mediator) {
		...
		if (noteInterests.length != 0) {
			// Create Observer
			BaseObserver observer = new BaseObserver(new BaseObserver.BaseFunction() {
	...
	/** Register an <code>Observer</code> to be notified of <code>INotifications</code> with a given name.
	 * 
	...
	public void registerObserver (String notificationName, Observer observer) {
		if (observerMap.get(notificationName) == null) {
			observerMap.put(notificationName, new ArrayList<Observer>());
		}
