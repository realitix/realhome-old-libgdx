 ...
 * <p>
 * The simplest way is to subclass </code>Facade</code>, and use its <code>initializeController</code> method to add your
 * registrations.
 * 
 * @see CoreView View
...
public class CoreController implements Controller {
	 ...
	 * <p>
	 * <p>
	 * This <code>IController</code> implementation is a Singleton, so you should not call the constructor directly, but instead
	 * call the static Singleton Factory method <code>Controller.getInstance()</code> */
	 ...
	 * <p>
	 * <P>
	 * Note that if you are using a subclass of <code>View</code> in your application, you should <i>also</i> subclass
	 * <code>Controller</code> and override the <code>initializeController</code> method in the following way:
	 * </P>
	 * <p>
	 * <listing> // ensure that the Controller is talking to my IView implementation override public function initializeController(
	 * ) : void { view = MyView.getInstance(); } </listing> */
	protected void initializeController () {
		view = CoreView.getInstance();
	}

	/** If an <code>ICommand</code> has previously been registered to handle a the given <code>INotification</code>, then it is
	 * executed.
	 * 
	 ...
	 * <p>
	 * <p>
	 * If an <code>ICommand</code> has already been registered to handle <code>INotification</code>s with this name, it is no
	 * longer used, the new <code>ICommand</code> is used instead.
	 * </P>
	 * <p>
	 * The Observer for the new ICommand is only created if this the first time an ICommand has been regisered for this
	 * Notification name.
	 * 
	...
	public void registerCommand (String notificationName, Class<? extends Command> command) {
		...
		view.registerObserver(notificationName, new BaseObserver(new BaseObserver.BaseFunction() {
