...
import com.realhome.util.clipper.Path.Join;
import com.realhome.util.clipper.Path.OutRec;
...
public class DefaultClipper extends ClipperBase {
	...
	// ------------------------------------------------------------------------------
	private static boolean joinHorz (Path.OutPt op1, Path.OutPt op1b, Path.OutPt op2, Path.OutPt op2b, LongPoint Pt,
		boolean DiscardLeft) {
	...
	public DefaultClipper (int InitOptions) // constructor
	{
		...
		/*
		 * intersectNodeComparer = ( node1, node2 ) -> { final long i = node2.getPt().getY() - node1.getPt().getY(); if (i > 0) {
		 * return 1; } else if (i < 0) { return -1; } else { return 0; } };
		 */
	...
	private void fixupOutPolygon (OutRec outRec) {
		...
		for (;;) {
			...
			// Testttstststs for duplicate points and collinear edges ...
	...
	private void insertLocalMinimaIntoAEL (long botY) {
		...
		while (currentLM != null && currentLM.y == botY) {
			...
			if (lb.outIdx >= 0 && lb.prevInAEL != null && lb.prevInAEL.getCurrent().getX() == lb.getBot().getX()
				&& lb.prevInAEL.outIdx >= 0 && Edge.slopesEqual(lb.prevInAEL, lb) && lb.windDelta != 0 && lb.prevInAEL.windDelta != 0) {
			...
			if (lb.nextInAEL != rb) {
				...
				if (rb.outIdx >= 0 && rb.prevInAEL.outIdx >= 0 && Edge.slopesEqual(rb.prevInAEL, rb) && rb.windDelta != 0
		...
		if (e1Contributing && e2Contributing) {
			...
			if (e1.polyTyp != e2.polyTyp) {
				...
				case UNION:
					...
					break;
				case DIFFERENCE:
					if (e1.polyTyp == PolyType.CLIP && e1Wc2 > 0 && e2Wc2 > 0 || e1.polyTyp == PolyType.SUBJECT && e1Wc2 <= 0
						&& e2Wc2 <= 0) {
						addLocalMinPoly(e1, e2, pt);
					}
					break;
	...
	private void processEdgesAtTopOfScanbeam (long topY) {
		...
		while (e != null) {
			if (e.isIntermediate(topY)) {
				...
				if (ePrev != null && ePrev.getCurrent().getX() == e.getBot().getX() && ePrev.getCurrent().getY() == e.getBot().getY()
					&& op != null && ePrev.outIdx >= 0 && ePrev.getCurrent().getY() > ePrev.getTop().getY()
					&& Edge.slopesEqual(e, ePrev) && e.windDelta != 0 && ePrev.windDelta != 0) {
					...
					&& eNext.getCurrent().getY() > eNext.getTop().getY() && Edge.slopesEqual(e, eNext) && e.windDelta != 0
					&& eNext.windDelta != 0) {
		...
		if (horzEdge.nextInLML != null) {
			if (horzEdge.outIdx >= 0) {
				...
				if (ePrev != null && ePrev.getCurrent().getX() == horzEdge.getBot().getX()
					&& ePrev.getCurrent().getY() == horzEdge.getBot().getY() && ePrev.windDelta != 0 && ePrev.outIdx >= 0
					&& ePrev.getCurrent().getY() > ePrev.getTop().getY() && Edge.slopesEqual(horzEdge, ePrev)) {
					...
					&& eNext.getCurrent().getY() == horzEdge.getBot().getY() && eNext.windDelta != 0 && eNext.outIdx >= 0
					&& eNext.getCurrent().getY() > eNext.getTop().getY() && Edge.slopesEqual(horzEdge, eNext)) {
