...
import com.realhome.util.clipper.Clipper.ClipType;
import com.realhome.util.clipper.Clipper.EndType;
import com.realhome.util.clipper.Clipper.JoinType;
import com.realhome.util.clipper.Clipper.PolyFillType;
import com.realhome.util.clipper.Clipper.PolyType;
import com.realhome.util.clipper.Point.DoublePoint;
...
public class ClipperOffset {
	...
	private void doMiter (int j, int k, double r) {
		...
		destPoly.add(new LongPoint((int)Math.round(srcPoly.get(j).getX() + (normals.get(k).getX() + normals.get(j).getX()) * q),
			(int)Math.round(srcPoly.get(j).getY() + (normals.get(k).getY() + normals.get(j).getY()) * q)));
	}
		...
		for (int i = 0; i < polyNodes.getChildCount(); i++) {
			...
			if (len == 1) {
				if (node.getJoinType() == JoinType.ROUND) {
					...
					for (int j = 1; j <= steps; j++) {
						destPoly.add(new LongPoint((int)Math.round(srcPoly.get(0).getX() + X * delta), (int)Math.round(srcPoly.get(0)
							.getY() + Y * delta)));
						final double X2 = X;
						X = X * cos - sin * Y;
						Y = X2 * sin + Y * cos;
			...
			if (node.getEndType() == EndType.CLOSED_POLYGON) {
				...
				if (node.getEndType() == EndType.OPEN_BUTT) {
					final int j = len - 1;
					pt1 = new LongPoint((int)Math.round(srcPoly.get(j).getX() + normals.get(j).getX() * delta),
						(int)Math.round(srcPoly.get(j).getY() + normals.get(j).getY() * delta), 0);
					destPoly.add(pt1);
					pt1 = new LongPoint((int)Math.round(srcPoly.get(j).getX() - normals.get(j).getX() * delta),
						(int)Math.round(srcPoly.get(j).getY() - normals.get(j).getY() * delta), 0);
					destPoly.add(pt1);
				...
				if (node.getEndType() == EndType.OPEN_BUTT) {
					pt1 = new LongPoint((int)Math.round(srcPoly.get(0).getX() - normals.get(0).getX() * delta),
						(int)Math.round(srcPoly.get(0).getY() - normals.get(0).getY() * delta));
					destPoly.add(pt1);
					pt1 = new LongPoint((int)Math.round(srcPoly.get(0).getX() + normals.get(0).getX() * delta),
						(int)Math.round(srcPoly.get(0).getY() + normals.get(0).getY() * delta));
					destPoly.add(pt1);
	...
	private void doRound (int j, int k) {
		final double a = Math.atan2(inA, normals.get(k).getX() * normals.get(j).getX() + normals.get(k).getY()
			* normals.get(j).getY());
		final int steps = Math.max((int)Math.round(stepsPerRad * Math.abs(a)), 1);

		double X = normals.get(k).getX(), Y = normals.get(k).getY(), X2;
		for (int i = 0; i < steps; ++i) {
			destPoly.add(new LongPoint((int)Math.round(srcPoly.get(j).getX() + X * delta), (int)Math.round(srcPoly.get(j).getY() + Y
				* delta)));
			X2 = X;
			X = X * cos - sin * Y;
			Y = X2 * sin + Y * cos;
		}
		destPoly.add(new LongPoint((int)Math.round(srcPoly.get(j).getX() + normals.get(j).getX() * delta), (int)Math.round(srcPoly
			.get(j).getY() + normals.get(j).getY() * delta)));
	}
	...
	private void doSquare (int j, int k) {
		...
		destPoly.add(new LongPoint((int)Math.round(sjx + delta * (nkx - nky * dx)),
			(int)Math.round(sjy + delta * (nky + nkx * dx)), 0));
		destPoly.add(new LongPoint((int)Math.round(sjx + delta * (njx + njy * dx)),
			(int)Math.round(sjy + delta * (njy - njx * dx)), 0));
	}
	...
	private void fixOrientations () {
		...
		if (lowest.getX() >= 0 && !polyNodes.childs.get((int)lowest.getX()).getPolygon().orientation()) {
			for (int i = 0; i < polyNodes.getChildCount(); i++) {
				...
				if (node.getEndType() == EndType.CLOSED_POLYGON || node.getEndType() == EndType.CLOSED_LINE
